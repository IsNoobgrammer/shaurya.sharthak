    ...
    >
      <svg viewBox="0 0 32 40" width="22" height="28" fill="none" aria-hidden="true">
        <ellipse
          ...
          rx="13"
          ry="17"
          fill={active ? 'url(#eggGlow)' : 'rgba(167,139,250,0.15)'}
    ...
    <section id="hero" className="hero">
      ...
      {!isMobile() && (
        <Suspense fallback={null}>
          <HeroScene gitaForeground={gitaMode} />
        </Suspense>
      )}

      {/* Egg easter egg button */}
      <EggButton active={gitaMode} onToggle={toggleGita} />

      {/* Hero content — fades when gitaMode is on */}
      <motion.div
      ...
      >
        ...
        >
          <button id="hero-view-projects" className="btn btn-primary" onClick={scrollToProjects}>
            View Projects
          </button>
          <button id="hero-get-in-touch" className="btn btn-secondary" onClick={scrollToContact}>
            Get in Touch
          </button>
        </motion.div>
      </motion.div>

      {/* Gita overlay text (foreground mode) */}
      <AnimatePresence>
        {gitaMode && (
          <motion.div
            className="gita-overlay"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
